...
print(f"[RESET] state.json cleared for {today}. All session locks removed.")
...
# 3. Count remaining LIVE trades today
cursor.execute("SELECT COUNT(*) FROM trades WHERE trade_date = ? AND execution_mode = 'LIVE'", (today,))
live_count = cursor.fetchone()[0]
conn.close()

print(f"[RESET] Deleted {deleted} DRY_RUN/test records from trades.db for {today}.")
print(f"[STATUS] Remaining LIVE trades today: {live_count}")
print("[READY] Session state reset complete. System ready for live trading.")
