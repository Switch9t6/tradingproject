...
def new_mon():
    return PositionMonitor(entry_premium=ENTRY, target_p=TARGET, initial_stop_p=STOP)
...
assert m.current_stop_p == ENTRY, m.current_stop_p
print("PASS: stop moved to breakeven:", m.current_stop_p)
...
assert abs(m.current_stop_p - round(ENTRY * 1.10, 2)) < 1e-9, m.current_stop_p
